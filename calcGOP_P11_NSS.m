function GOP_NSS = calcGOP_P11_NSS(d17O_O2,d18O_O2,SP,pt,kO2,h,O2,dD17dt,varargin)
% calcGOP_P11_NSS: steady state gross oxygen production using Prokopenko
% 2011 non-steady state equation S8
% -------------------------------------------------------------------------
% USAGE:
% -------------------------------------------------------------------------
% GOP = calcGOP_P11_NSS(d17O,d18O,SP,pt,kO2,mld,O2,dD17dt)
% GOP = calcGOP_P11_NSS(-0.5712,-1.2779,35,15,1.4,14,0.250,5)
%   > GOP = 0.3040
% or
% GOP = calcGOP_P11_NSS(d17O,d18O,S,T,k,mld,O2,dD17dt,d17O_H2O,d18O_H2O)
% GOP = calcGOP_P11_NSS(-0.5712,-1.2779,35,15,1.4,14,0.250,5,-12.1885,-23.8867) 
%   > GOP = 0.3327
%
% -------------------------------------------------------------------------
% INPUTS:
...
% O2        = O2 concentration (mol m^-3) 
% dD17dt    = rate of change in D17 with time (per meg d^-1)
...
% OUTPUTS:
% -------------------------------------------------------------------------
% GOP_NSS  = non-steady state gross oxygen production rate in mol O2 m^2 d^-1
%
% -------------------------------------------------------------------------
...
%
% This function uses equation S8 from Prokopenko et al. (2011) Geophys.
% Res. Lett. Note that as of August 1, 2016, equation 6 in the main
% manuscript has typos but equation S8 is correct. An erratum correcting
% this typo is in progress.
% 
...
% equation as written above (Hamme et al. (2012))
%
% Non-steady state GOP is calculated as:
%
%        kO2 * [O2]eq * [(X17 - X17air*a17eq)/X17 - lambda*(X18 - X18air*a18eq)/X18] + h * [O2] * dD17/dt
% GOP = -----------------------------------------------------------------------------------------------------
...
% for air-sea gas exchange, a17p is the fractionation factor for
% photosynthesis, and lambda is the ratio of 17O and 18O fractionation
% factors for respiratory O2 consumption, h is the mixed layer depth, and
% dD17/dt is the rate of change in D17 with time.
%
% NOTES (see function below):
%
% 1) Uses isotopic composition of VSMOW as defined in Barkan and Luz (2011)
% Rapid Comm. Mass Spectrom., Table 1 average 2007-2009 with respect to
% atmospheric O2.
%
...
% terms in numerator
N1 = kO2.*gasmoleq(SP,pt,'O2').*((X17 - X17air*a17eq)./X17 - 1*lambda*(X18-X18air*a18eq)./X18);
N2 = h.*O2.*dD17dt./1e6;
